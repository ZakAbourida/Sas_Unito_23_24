...
public class TestSheet4b {
    ...
    public static void main(String[] args) {
        try {
            ...
            // Caricare tutti i turni disponibili
            System.out.println("\nALL TURNS");
            List<Turn> allTurns = CatERing.getInstance().loadAllTurnForService(selectedService.getId());
            for (int i = 0; i < allTurns.size(); i++) {
                System.out.println((i + 1) + ": " + allTurns.get(i).toString());
            }

            System.out.print("Seleziona il numero del nuovo turno: ");
            int turnIndex = scanner.nextInt() - 1;
            Turn newTurn = allTurns.get(turnIndex);

            CatERing.getInstance().getSummarySheetManager().modifyTurn(newTurn, selectedAssignment);
